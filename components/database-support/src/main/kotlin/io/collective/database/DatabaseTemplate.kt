...
class DatabaseTemplate(private val dataSource: DataSource) {
    ...
    fun <T> findByField(sql: String, mapper: (ResultSet) -> T, field: String): T? {
        dataSource.connection.use { connection ->
            return findByField(connection, sql, mapper, field)
        }
    }

    fun <T> findById(connection: Connection, sql: String, mapper: (ResultSet) -> T, id: Long): T? {
        val list = query(connection, sql, { ps -> ps.setLong(1, id) }, mapper)
        return list.firstOrNull()
    }

    fun <T> findByField(connection: Connection, sql: String, mapper: (ResultSet) -> T, field: String): T? {
        val list = query(connection, sql, { ps -> ps.setString(1, field) }, mapper)
        return list.firstOrNull()
    }
